...
const database = new (require('./lib/database'))() // Sesuaikan dengan path lib kamu

  const loadData = database.read();
  if (loadData && Object.keys(loadData).length === 0) {
    ...
    database.write(global.db).catch(e => console.error(e));
...
async function sockConnect() {
 ...
 if (!sock.authState.creds.registered) {
  ...
  store.bind(sock.ev)
      // Tambahkan ini agar sock mengenali fungsi decodeJid
    sock.decodeJid = (jid) => {
        if (!jid) return jid
        if (/:\d+@/gi.test(jid)) {
  ...
  sock.ev.on('messages.upsert', async (chatUpdate) => {
    try {
        ...
        // Supaya bot bisa baca pesan ephemeral/sekali lihat
        mek.message = (Object.keys(mek.message)[0] === 'ephemeralMessage') 
            ? mek.message.ephemeralMessage.message 
            : mek.message
        
        // Abaikan status WA (broadcast)
        if (mek.key && mek.key.remoteJid === 'status@broadcast') return
        
        // FIX LOGIKA PUBLIC: Ambil dari variabel 'mode' baris 118
        if (!mode.public && !mek.key.fromMe) return 

        // Format pesan agar bisa dibaca message.js
        const m = smsg(sock, mek, store)

        // Catat user aktif di database
        if (!global.db.active) global.db.active = {}
        global.db.active[m.sender] = Date.now()

        // PANGGIL HANDLER PESAN
        require("./message")(sock, m, chatUpdate, store)

    } catch (err) {
        console.error("Error di Handler Chat:", err)
    }
})
